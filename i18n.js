...
const KO_DICT = [
  ...
  ['.trust-label', '찾기'],
  ['.hero-trust a[href*="vibecamp"]', '기업교육 · Vibecamp'],
  ...
  ['#onprem .onprem-grid .onprem-card:nth-child(6) h3', '잘된 실행이 기록·문서·업무 방식으로 남습니다.'],
  ...
  ['#onprem .tools-k', '부속 도구'],
  ['#onprem .tools .tool-card:nth-child(1) .tool-t', 'AI 활용 성숙도 진단'],
  ['#onprem .tools .tool-card:nth-child(1) .tool-d', '24문항으로 현재 AI 활용 방식을 11단계로 확인합니다.'],
  ['#onprem .tools .tool-card:nth-child(2) .tool-t', '업무 데이터 비식별화 도구'],
  ['#onprem .tools .tool-card:nth-child(2) .tool-d', '사내 자료를 AI에 쓰기 전, 브라우저 안에서 가명처리·마스킹합니다 (서버 전송 없음).'],
  ['#onprem .tools .tool-card:nth-child(3) .tool-t', '터미널에서도 · CLI'],
  ...
  ['#svc .section-head', '시작하는 방법은<br/><em>두 가지</em>입니다.'],
  ['#svc .section-sub', '좌석 온보딩으로 바로 시작하거나, 저희가 먼저 만들어 돌린 결과를 보고 계약할 수 있습니다.'],
  ['#svc .mk-1', '방법 1 · 좌석 온보딩'],
  ['#svc .mk-2', '방법 2 · 선개발 스프린트'],
  ['#svc .sprint > h3', '저희가 먼저 만들어 돌립니다. 굴러가는 것을 보고 계약하세요.'],
  ['#svc .sprint-steps .sp-step:nth-child(1) .sp-num', '1단계'],
  ['#svc .sprint-steps .sp-step:nth-child(1) .sp-t', '선개발 스프린트'],
  ['#svc .sprint-steps .sp-step:nth-child(1) .sp-d', '1~2주. 귀사의 실데이터와 실제 문제로 동작하는 결과물을 만듭니다.'],
  ['#svc .sprint-steps .sp-step:nth-child(2) .sp-num', '2단계'],
  ['#svc .sprint-steps .sp-step:nth-child(2) .sp-t', '성과 검증'],
  ['#svc .sprint-steps .sp-step:nth-child(2) .sp-d', '도입 전 기준선을 함께 측정하고 지표를 확정합니다.'],
  ['#svc .sprint-steps .sp-step:nth-child(3) .sp-num', '3단계'],
  ['#svc .sprint-steps .sp-step:nth-child(3) .sp-t', '성과연동 정산'],
  ['#svc .sprint-steps .sp-step:nth-child(3) .sp-d', '성과가 난 만큼 정산합니다. 월 최소액과 연간 상한이 양측을 보호합니다.'],
  ['#svc .sprint-cond .sc-l', '세 가지가 갖춰진 경우에만 착수합니다'],
  ['#svc .sprint-cond .jr-checks li:nth-child(1)', '실데이터 제공'],
  ['#svc .sprint-cond .jr-checks li:nth-child(2)', '숫자로 측정할 수 있는 업무'],
  ['#svc .sprint-cond .jr-checks li:nth-child(3)', '의사결정권자 참여'],
  ['#svc .sprint-safe .pipe-chip:nth-child(1) .pc-t', '성과가 0이면 운영비만 정산'],
  ['#svc .sprint-safe .pipe-chip:nth-child(2) .pc-t', '연간 상한 있음'],
  ['#svc .sprint-safe .pipe-chip:nth-child(3) .pc-t', '정산 이의 절차 있음'],
  ['#svc .sprint .btn', '선개발 스프린트 문의 →'],
  ...
  ['#svc .journey-card.featured .jr-checks li:nth-child(1)', '도입 효과 리포트 — 운영 전후 동일 지표 비교'],
  ...
  ['#svc .rm-title', '도입 효과 리포트'],
  ['#svc .rm-sub', '— 기준선부터 전후 비교까지, 숫자는 귀사의 실행이 채웁니다'],
  ...
  ['.inquiry-form select option:nth-child(3)', '선개발 스프린트'],
  ['.inquiry-form select option:nth-child(4)', '교육 연계'],
  ['.inquiry-form select option:nth-child(5)', '제휴'],
  ['.inquiry-form select option:nth-child(6)', '기타'],
  ['.iq-submit', '문의 보내기 →'],
  ...
  ['.faq-list details:nth-of-type(2) p',
    '두 단계입니다 — 당일 시작하는 좌석 단위 <strong>단기 온보딩</strong>, 그리고 도입 효과 리포트가 포함된 <strong>연간 구독</strong>. 견적은 조직 규모에 따라 <a href="service.html#inquiry">도입 상담</a>에서 안내합니다.'],
  ['.faq-list details:nth-of-type(3) summary', '우리 데이터는 어떻게 관리되나요?'],
  ['.faq-list details:nth-of-type(3) p',
    '대화와 데이터는 좌석·프로젝트 단위로 분리되고, 조직 문서 조회는 프로젝트 격리 안에서 동작하며, 개인정보 차단 정책이 적용됩니다. 상세 보안 자료와 질의서 답변은 도입 상담에서 제공합니다.'],
  ['.faq-list details:nth-of-type(4) summary', '계약이 끝나면 쌓인 기록은 어떻게 되나요?'],
  ['.faq-list details:nth-of-type(4) p',
    '계약 종료 시 데이터 처리 방식을 계약서에 명시하고, 도입 상담에서 미리 안내합니다.'],
  ['.faq-list details:nth-of-type(5) summary', '회사 기존 툴체인과 같이 쓸 수 있나요?'],
  ...
  ['.faq-list details:nth-of-type(6) summary', '채용 중인가요?'],
  ['.faq-list details:nth-of-type(6) p',
